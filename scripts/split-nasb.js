...
const INPUT_PATH = path.join(ROOT_DIR, 'NASB.json');
const OUTPUT_ROOT = path.join(ROOT_DIR, 'assets', 'nasb');
...
async function main() {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error('NASB.json must contain a top-level array');
  }

  const verses = parsed.map(normalizeVerse).sort(sortVerses);
  if (verses.length === 0) {
    throw new Error('NASB.json contains no verses');
  }

  const translationSet = new Set(verses.map((v) => v.translation));
  if (translationSet.size !== 1 || !translationSet.has('NASB')) {
    throw new Error(`Expected exactly one translation "NASB", got: ${[...translationSet].join(', ')}`);
  }
  ...
  console.log(`Generated ${index.chapterCount} chapter files across ${index.bookCount} books in assets/nasb`);
}
